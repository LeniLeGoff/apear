...
torch::Tensor CPGCellImpl::forward(){
    _neuron_states = torch::tanh(torch::matmul(_weights,_neuron_states)) + _biases;
...
CPGCell2Impl::CPGCell2Impl(double init_state_0,double init_state_1){
    _neuron_states = torch::tensor({init_state_0,init_state_1},
                                   torch::TensorOptions().dtype(torch::kDouble));
    _weights = torch::empty({2,2},
                            torch::TensorOptions().dtype(torch::kDouble));
...
torch::Tensor CPGCell2Impl::forward(torch::Tensor x){
    _neuron_states[0] = x;
    _neuron_states = torch::tanh(torch::matmul(_weights,_neuron_states)) + _biases;
...
void CPGCell2Impl::set_weights_biases(std::vector<double> weights, std::vector<double> biases){
    using namespace torch;
    set_weights_biases(torch::from_blob(weights.data(),
                                        {static_cast<int64_t>(weights.size())},
                                        TensorOptions().dtype(kDouble)).clone(),
...
void CPGCell2Impl::set_weights_biases(const torch::Tensor& weights, const torch::Tensor& biases){
    using namespace torch;
    _weights = reshape(weights.clone(),{2,2});
    _biases = biases.clone();
    ...
    _rbf(2,nbr_hidden), _out(nbr_hidden, _nbr_outputs), _init_state(init_state)
{
    ...
    _in->to(torch::kDouble);
    std::vector<double> inputs_weights(2,0.1);
    for(int i = 0; i<4;i++)
        inputs_weights.push_back(1);
    _in->weight = reshape(torch::from_blob(
            inputs_weights.data(),
        {static_cast<int64_t>(2*2)},
        torch::TensorOptions().dtype(torch::kDouble)).clone(),
        {2,2});
    _in->bias = torch::zeros(2,torch::TensorOptions().dtype(torch::kDouble));
    _out->to(torch::kDouble);
    _rbf->to(torch::kDouble);
}
...
void CPGRBFNetwork::init_cpg(double alpha, double phi){
    _cpg->as<CPGCell>()->set_weights_biases({alpha*cos(phi*M_PI),alpha*sin(phi*M_PI),
                              -alpha*sin(phi*M_PI),alpha*cos(phi*M_PI)},
                             {0,0});
}
void CPGRBFNetwork::init_rbf(){
    torch::Tensor x;// = torch::full({_nbr_inputs},{_init_state},torch::kDouble);
    std::vector<double> centers;
    while(centers.size() < static_cast<size_t>(_nbr_hidden*2)){
        torch::Tensor y;
        y = _cpg->as<CPGCell>()->forward();
        centers.insert(centers.end(),y.data_ptr<double>(),y.data_ptr<double>()+2);
    }
    _rbf->set_centers(centers);
...
void CPGRBFNetwork::set_cpg_parameters(std::vector<double> weights, std::vector<double> biases){
    _cpg->as<CPGCell>()->set_weights_biases(weights,biases);
}
...
void MLPControl::init_nn(int nbr_inputs, int nbr_outputs, int nbr_hidden){
    _nn = std::make_shared<MLP>(nbr_inputs,nbr_outputs,nbr_hidden);
}
...
void RNNControl::init_nn(int nbr_inputs, int nbr_outputs, int nbr_hidden){
    _nn = std::make_shared<RNN>(nbr_inputs,nbr_outputs,nbr_hidden);
}
...
void CPGRBFRNNControl::init_nn(int nbr_inputs,int nbr_outputs, int nbr_rbf, int nbr_rnn, double init_state_0, double init_state_1, double alpha, double phi){
    _nn = std::make_shared<CPGRBFRNN>(nbr_inputs,nbr_rbf, nbr_rnn, nbr_outputs, init_state_0, init_state_1);
    _nn->init_cpg(alpha, phi);
    _nn->init_rbf();
}
