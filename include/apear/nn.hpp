...
struct CPGCellImpl: torch::nn::Module{
    ...
    void set_weights_biases(std::vector<double> weights, std::vector<double> biases);
    void set_weights_biases(const torch::Tensor& weights, const torch::Tensor& biases);
    const torch::Tensor &neuron_states() const {return _neuron_states;}
 ...
 * "SO(2)-Networks as Neural Oscillators", F. Pasemann, 2003, In International work-conference on artificial neural networks
 */
struct CPGCell2Impl: torch::nn::Module{
    CPGCell2Impl(double init_state_0,double init_state_1);
    torch::Tensor forward(torch::Tensor x);
...
public:
    ...
    void set_randonNum(const misc::RandNum::Ptr& rn){_rand_num = rn;}

    std::shared_ptr<nn::MLP> _nn;
};
...
public:
    ...
    std::shared_ptr<nn::CPGRBFNetwork> _nn;
};
...
public:
    ...
    std::shared_ptr<nn::CPGRBFRNN> _nn;
};
